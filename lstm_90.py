...
class LSTMModel(nn.Module):
    def __init__(self, num_features, num_outputs, dropout_rate=0.3):
        super().__init__()
        self.lstm1 = nn.LSTM(input_size=num_features, hidden_size=128, num_layers=2, batch_first=True, dropout=dropout_rate)
        self.dropout1 = nn.Dropout(dropout_rate)
        self.lstm2 = nn.LSTM(input_size=128, hidden_size=64, num_layers=1, batch_first=True)
        self.dropout2 = nn.Dropout(dropout_rate)
        self.shortcut = nn.Linear(num_features, num_outputs)
